...
export function useListbox<T>(
  ...
  // The main change is in the useControlledSelection function
  const {selectedValues, selectValues} = useControlledSelection(props);

  // ... existing code ...

  // Modify the handleItemSelection function for multi-selection
  const handleItemSelection = useCallback(
    (value: PrimitiveValue) => {
      // ... existing code ...

      // Change logic for multiple selection
      if (selectionMode === 'multiple') {
        let newSelectedValues;
        if (selectedValues.includes(value)) {
          // Remove value from selectedValues if it's already selected
        ...
        // Single selection logic remains the same
        selectValues(value);
      }

      // ... existing code ...
    },
    [selectedValues, selectValues, selectionMode]
  );
  ...
  return {
    ...
    state: {
      ...
      selectedValues, // Ensure selectedValues is part of the returned state
...
function useControlledSelection(props: ListboxProps) {
  ...
  const selectValues = useCallback(
    (mixedValue: PrimitiveValue | PrimitiveValue[] | null) => {
      ...
      if (selectionMode === 'multiple' && Array.isArray(mixedValue)) {
        setStateValues(mixedValue);
      } else {
        // Handle single selection
        const newValue = Array.isArray(mixedValue) ? mixedValue[0] : mixedValue;
        setStateValues(newValue);
      }
    },
    [setStateValues, selectionMode]
  );
